...
case: (Order.TotalPOrderMixin.leP letot v (F y)) => [le|lt].
  exists y; rewrite !(in_cons, eqxx) orbT /= !big_cons joinCA join_l//.
...
rewrite !big_cons joinCA join_r//.
...
move: (mleadD_le p1 p2); rewrite join_r ?(ltW lt_p1p2)// => ->.
...
rewrite join_l => //; apply/joinsP_seq=> {m} /= m.
